...
| ユニット（184） | ✅ 全通過 |
| 統合（97） | ✅ 全通過 |
| ゴールデン（63） | 47 pass / 16 fail（既知の互換ギャップ） |
...
ゴールデンテスト 16 件の不一致は 68020+ EA・最適化レベル差異等。詳細は [verification_backlog.md](verification_backlog.md) 参照。
...
## 直近コミット（ドキュメント時点）
- ゴールデンテスト ERROR 上位3件修正（4テスト pass 化）: ネストマクロ、ビットフィールド、CAS2/DIVSL/DIVUL/MULS.L長形式
