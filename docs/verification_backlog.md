...
### 3. FBcc/FDBcc 外部参照ターゲット ✅
- 対象: FPU 分岐命令（`fbcc`/`fdbcc`）のターゲットが XREF（外部参照）の場合
- 状態: **対応済み** (`98fb1b3`, `1d75cb2`)。`.w` は原典同様 `0x65` ワードオフセットリロケーション。`.l` は原典 `outpcdisp7` 相当の RPN リロケーション（`$80FF` xref + `$80xx` base + `$A00F` sub + `$9200` long）。統合テスト 4 件追加

### 4. Bcc.L 外部参照 PC 相対リロケーション ✅
- 対象: `.l` 形式の PC 相対分岐で外部参照先を指定した場合
- 状態: **対応済み** (`1d75cb2`)。原典 `outpcdisp7` 相当の RPN リロケーションを生成。`emit_pc_rel_rpn()` ヘルパーで Bcc.L / FBcc.L 共通化。統合テスト 2 件追加
