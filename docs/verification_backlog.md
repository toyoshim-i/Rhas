...
- `integration_test`: 88/88 pass
- `error_message_test`: 8/8 pass
- MS6（FPU/SCD/残互換機能）実装済み
- 優先度A/Bで追跡していた互換性検証項目は完了
- `error.rs` テーブル経由エラー出力: pass1 完了
- dead_code 警告ゼロ達成

## 残互換ギャップ（機能未接続）

### 1. ColdFire CPU 選択ディレクティブ（優先度：中）
- 対象: `.5200` / `.5300` / `.5400`
- 状態: `InsnHandler::Cpu5200/Cpu5300/Cpu5400` は定義済み、CPU 定数（`cpu::C520` 等）も存在するが `handle_pseudo` で未接続（`_ => {}` にフォールスルー）
- 影響: ColdFire ソースのアセンブルで CPU 状態が更新されない
- 対応: `.68000`〜`.68060` と同一パターンで数行追加

### 2. `.cpu` 文字列パラメータ処理（優先度：低）
- 対象: `.cpu 68020` 等の文字列指定形式
- 状態: `InsnHandler::Cpu` のハンドラ内でパラメータ処理が省略（コメントのみ）
- 影響: `.cpu <name>` 形式を使用するソースで CPU 状態が更新されない
- 補足: 数値形式（`.68020` 等）は正常動作。`options.rs:871` に文字列→定数の変換テーブルは存在

### 3. FBcc/FDBcc 外部参照ターゲット（優先度：低）
- 対象: FPU 分岐命令（`fbcc`/`fdbcc`）のターゲットが XREF（外部参照）の場合
- 状態: `pass3.rs:890,949` でゼロ埋め＋エラーカウント増加
- 影響: FPU 分岐で外部ラベルを参照するケースは稀。実プログラム比較で不一致は出ていない
- 方針: 原典 HAS での挙動を確認後に判断（非対応維持 or PC 相対リロケーション生成）

### 4. Bcc.L 外部参照 PC 相対リロケーション（優先度：低）
- 対象: `.l` 形式の PC 相対分岐で外部参照先を指定した場合
- 状態: `pass3.rs:1217-1221` で advance＋エラーカウント増加。`.w`/`.s` 形式は対応済み
- 影響: `.w` で代替可能。実プログラム比較で不一致は出ていない
- 方針: 原典 HAS で `.l` 外部参照分岐が生成するリロケーション形式を確認後に判断
