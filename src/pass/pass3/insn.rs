...
use super::P3Ctx;
use super::ea::{EaExtKind, resolve_ea_with_ext, ea_ext_size_for_insn};
use super::{emit_pc_rel_rpn, emit_abs_xref, emit_rofst, emit_rpn_expression};
...
pub(super) fn process_deferred(
    ...
    if matches!(handler, InsnHandler::DBcc) {
        ...
        let dn = match resolved_ops.first() {
            ...
            _ => { ctx.emit_zeros(4); ctx.error_code(ErrorCode::IlAdr, None); return; }
        };
    ...
    if matches!(handler, InsnHandler::FDBcc) {
        ...
        let dn = match resolved_ops.first() {
            Some(EffectiveAddress::DataReg(n)) => *n,
            _ => { ctx.emit_zeros(6); ctx.error_code(ErrorCode::IlAdr, None); return; }
        };
