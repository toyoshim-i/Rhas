...
| SCD デバッグ情報（`-g`） | 🚧 進行中 | `.file/.ln/.def/.endef/.val/.scl/.type/.tag/.line/.size/.dim` の構文/値検証 + `TempRecord`化 + `ObjectCode.scd_events` 収集 + `$0000` 後ろの SCD フッタ出力骨格まで実装。`B204`のファイル名に`.file`を反映。HAS 完全一致へ調整中 |
