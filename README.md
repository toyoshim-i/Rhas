...
| MS6 | FPU（68881/68882）/ SCD デバッグ情報 / 残互換機能（`.fpid`/`.offsym` などを段階的に実装） | 🚧 進行中 |
...
| SCD デバッグ情報（`-g`） | 🚧 進行中 | `.file/.ln/.def/.endef/.val/.scl/.type/.tag/.line/.size/.dim` の構文/値検証 + `TempRecord`化 + `ObjectCode.scd_events` 収集 + `$0000` 後ろの SCD フッタ出力骨格まで実装。SCD フッタ `.file` 名と `B204` は入力ソース名を使用（`.file` 疑似命令名は有効化用途）。HAS 完全一致へ調整中 |
