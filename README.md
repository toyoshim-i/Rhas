...
| MS5 | 実 X68000 プログラムのビルドがオリジナルと完全一致 | 🔄 進行中（17/17 ファイル中 13 一致） |
| MS6 | FPU（68881/68882）/ SCD デバッグ情報 / PRN 全機能 | 📋 計画中 |
...
| HLK オブジェクトファイル出力 | ✅ 完了 | |
| PRN リストファイル出力 | ✅ 完了 | |
| `-c4` 拡張最適化（`ADD #1-8` → `ADDQ` 等） | 🔄 部分実装 | `opt_adda_suba` のみ。他 11 フラグは未実装 |
| FPU 命令（68881/68882） | 📋 計画中 | |
| SCD デバッグ情報（`-g`） | 📋 計画中 | スタブのみ |
